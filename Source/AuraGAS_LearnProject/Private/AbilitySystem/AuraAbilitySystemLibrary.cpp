...
void UAuraAbilitySystemLibrary::ApplyEffectToSelf(UAbilitySystemComponent* ASC, TSubclassOf<UGameplayEffect> GameplayEffectClass, float Level)
{
...
void UAuraAbilitySystemLibrary::InitializeDefaultAttributesFromSaveData(const UObject* WorldContextObject, UAbilitySystemComponent* ASC, ULoadMenuSaveGame* SaveGame)
{
	...
	/* Binding to Primary Attributes Change */
...
void UAuraAbilitySystemLibrary::SetKnockbackForceToDamageEffectParams(FDamageEffectParams& DamageEffectParams, FVector KnockbackDirection, float Magnitude)
{
...
void UAuraAbilitySystemLibrary::SetDeathImpulseToDamageEffectParams(FDamageEffectParams& DamageEffectParams, FVector ImpulseDirection, float Magnitude)
{
...
{
	// 这里不能用 Cast<>，只能用 static_cast，因为 FAuraGameplayEffectContext 不是 UObject
	if (const FAuraGameplayEffectContext* AuraEffectContext = static_cast<const FAuraGameplayEffectContext*>(EffectContextHandle.Get()))
	{
...
{
	const TFunction<bool(const AActor&, const AActor&)> DistancePredicate = [Origin](const AActor& A, const AActor& B)
	{
	...
	if (Actors.Num() <= MaxTargets)
	{
		...
		ActorsToCheck.Sort(DistancePredicate);
		OutClosestTargets = ActorsToCheck;
		return;
	}
	
	// Maintains a max-heap of size MaxTargets (farthest on top)
	for (AActor* Actor : Actors)
	{
		OutClosestTargets.HeapPush(Actor, DistancePredicate);
		if (OutClosestTargets.Num() > MaxTargets) OutClosestTargets.HeapPopDiscard(DistancePredicate);
	}
}
