...
{
	// As ClientEffectApplied is a Client RPC, once this delegate is broadcast on the Server, it will be called in the Server and executed on the Client
...
void UAuraAbilitySystemComponent::AbilityInputTagReleased(const FGameplayTag& InputTag)
{
	...
	for (FGameplayAbilitySpec& AbilitySpec : GetActivatableAbilities())
	{
		if (AbilitySpec.DynamicAbilityTags.HasTagExact(InputTag) && AbilitySpec.IsActive())
		{
			...
			// Send data to the Server and tell it that we're doing something
...
// Update ability statuses when level up
void UAuraAbilitySystemComponent::UpdateAbilityStatuses(int32 Level)
{
	...
	for (const FAuraAbilityInfo& Info : AbilityInfo->AbilityInformation)
	{
		...
		if (Level < Info.LevelRequirement) continue; // 检查是否达到等级要求
		if (GetSpecFromAbilityTag(Info.AbilityTag) == nullptr) // 检查是否已经获得Ability
		{
			FGameplayAbilitySpec AbilitySpec = FGameplayAbilitySpec(Info.Ability, 1);
			AbilitySpec.DynamicAbilityTags.AddTag(FAuraGameplayTags::Get().Abilities_Status_Eligible);
			GiveAbility(AbilitySpec);
			MarkAbilitySpecDirty(AbilitySpec); // Force an ability spec to replicate now instead of waiting till the next update
			
			ClientUpdateAbilityStatus(Info.AbilityTag, FAuraGameplayTags::Get().Abilities_Status_Eligible, 1); // 向WidgetController广播技能状态变化
		}
	}
}
...
{
	// ServerRPC: Client调用, Server执行, 执行权威行为
	if (FGameplayAbilitySpec* AbilitySpec = GetSpecFromAbilityTag(AbilityTag))
	{
		const FAuraGameplayTags& GameplayTags = FAuraGameplayTags::Get();
		...
		if (bStatusValid)
		{
			...
			if (FGameplayAbilitySpec* SpecWithSlot = GetSpecWithSlot(Slot)) // There is an ability in this slot already
			{
			...
			if (!AbilityHasAnySlot(*AbilitySpec)) // Ability doesn't yet have a slot (it's not active) 
			{
				...
				{
					bool bSuccess = TryActivateAbility(AbilitySpec->Handle);
					if (bSuccess) MulticastActivatePassiveEffect(AbilityTag, true);
				}
			}
			// AssignSlotToAbility
