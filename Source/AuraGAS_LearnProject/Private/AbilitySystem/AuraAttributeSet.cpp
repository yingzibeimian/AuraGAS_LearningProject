...
void UAuraAttributeSet::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	...
	// REPNOTIFY_Always: Whether value are changed, repnotify always
...
void UAuraAttributeSet::HandleIncomingDamage(const FEffectProperties& Props)
{
	...
	if (LocalIncomingDamage > 0.f)
	{
		...
		{
			// 不在IsBeingShocked状态时, 激活HitReact Ability
...
{
	// Inform the ASC that were replicating a changed value just come down from the Server,
	// so that ASC can register that change and keep track of its old value in case anything needs to be rolled back
	// (In case of perdiction, the Server can roll changes back if it deems that those are not valid changes)
